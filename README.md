...
Web app per lista della spesa **e piano alimentare settimanale**: nessuna build, funziona su un semplice hosting statico. Login e sincronizzazione cloud multi-dispositivo (opzionali) richiedono in più solo PHP per `api/sync.php`, senza database: senza login l'app resta comunque completamente utilizzabile in locale. Pensata per funzionare sia su un **Nokia Lumia 735** (Windows 10 Mobile, Edge 14 / EdgeHTML 14) sia su browser moderni (Android, iOS, desktop).

L'app è una Single Page Application (nessun reload di pagina tra una vista e l'altra), con navigazione a icone (senza etichette testuali) ancorata **fissa in fondo allo schermo** (più comoda da raggiungere col pollice su mobile) su 5 destinazioni principali: **Lista della spesa** (che include anche il catalogo prodotti, in un accordion in fondo alla pagina), **Piano alimentare** (settimanale, con ricette e generazione automatica della lista), **Oggi** (il piano di oggi e domani, sempre due colonne affiancate anche su telefono, vedi sotto), **Ricette** e **Impostazioni** (solo tema scuro + backup). Le due schermate di gestione categorie (corsie della lista/prodotti, e categorie ricetta) non sono nel menu principale: si aprono con un bottone dedicato rispettivamente dentro Lista e dentro Ricette, per non affollare la navigazione. Ogni vista ha un titolo `<h1>` (visivamente piccolo, stile `.h5`) per orientarsi facilmente ora che il menu non ha più etichette testuali.

## Come funziona

- Tutti i dati (lista, categorie, prodotti, ricette, piano alimentare) vivono in **localStorage** del browser: nessun server/database richiesto per l'uso base dell'app. La sincronizzazione cloud (opzionale, vedi sotto) è l'unica funzionalità che richiede un backend.
...
- **Account e sincronizzazione cloud** (opzionale, in "Impostazioni"): il login avviene tramite un hub di autenticazione esterno il cui **dominio è un campo libero** compilato dall'utente ("Dominio di sincronizzazione", salvato in `localStorage` come `shopping-kart-auth-hub-domain-v1`) — non un servizio fisso, così chiunque ospiti un hub compatibile con questo stesso protocollo può usarlo. Il login è un redirect a `https://<dominio>/login?client=...` (nessun form di login proprio) — vedi `js/auth.js`. Una volta autenticato, `js/sync.js` confronta i dati locali con quelli salvati sul backend proprio di Shopping Kart (`api/sync.php`, un file JSON per utente in `api/data/`) usando un timestamp `aggiornatoIl`: se solo un lato è cambiato aggiorna l'altro in automatico, se sono cambiati entrambi in modo indipendente mostra un confronto e lascia scegliere all'utente quale versione tenere. Ad ogni chiamata il client inoltra anche il dominio configurato (`authDomain`), che `api/sync.php` usa per validare il token contro quello specifico hub (`https://<authDomain>/api/user`, chiamata server-to-server) — mai fidandosi di un'identità dichiarata dal client. La sync è automatica: parte all'avvio, dopo ogni modifica locale (con un breve debounce), quando l'app torna in primo piano (`visibilitychange`/`focus`) e periodicamente mentre resta aperta, così non serve mai aggiornare a mano — l'icona di sync ruota mentre una chiamata è in corso, senza toast, e resta comunque disponibile un "Sincronizza ora" per un tentativo esplicito col risultato mostrato in un toast. Su browser senza `fetch` (Edge 14/Lumia) la sync resta semplicemente disattivata, il resto dell'app funziona come sempre.
...
- **Cella pasto condivisa (`meal-cell`)**: il contenuto di una cella Colazione/Pranzo/Cena (badge voci + bottone aggiungi) è un componente Vue a sé (`js/components.js`), riusato sia da `day-row` nella vista Piano (3 colonne strette per riga-giorno) sia dalla vista Oggi (3 sezioni impilate per colonna-giorno) — stesso comportamento, solo il layout del genitore cambia.
- **Vista Oggi: due colonne Oggi/Domani**: `oggiData`/`domaniData` (computed in `js/feature-piano.js`, la seconda semplicemente `DataModel.addDays(oggiData, 1)`) alimentano due blocchi affiancati (`col-6` fisso, classe Bootstrap senza breakpoint: sempre due colonne, anche da mobile — niente `col-sm-6`/impilamento). Ogni colonna è indipendente: propria selezione ricetta/nota, proprio drag&drop (stesso `group` Sortable delle altre celle, quindi si trascina anche direttamente da Oggi a Domani). Colonne strette anche su telefono significa badge stretti: e' il motivo per cui le azioni di ogni voce (sotto) collassano in un menu da mobile invece di restare 3 icone fisse.
- **Voci del piano: ricette o note libere**: ogni cella pasto è un array di "voci" (`{id, tipo:'ricetta', ricettaId, nota}` oppure `{id, tipo:'nota', testo}`, vedi commento in testa a `js/feature-piano.js`), non più solo ricette. Una nota "libera" (`tipo:'nota'`) è testo libero (es. "Pizza con amici fuori") per i pasti che non vale la pena modellare come ricetta/ingredienti veri: si aggiunge dallo stesso modale "Aggiungi al piano" (campo separato sotto la ricerca ricetta), si modifica con un tap sul testo (editor inline nella cella), ed è fine a se stessa — non genera ingredienti in "Genera lista" e se rimossa dal piano è persa per sempre (nessun salvataggio in un catalogo, a differenza delle ricette). Il campo `nota` su una voce-ricetta è invece una breve annotazione sulla singola istanza pianificata (es. "x2" per una doppia porzione, mostrata in corsivo/small dopo il nome ricetta nel badge): due voci che puntano alla stessa ricetta possono avere note diverse o nessuna, la ricetta in sé resta unica nel catalogo. Si aggiunge/modifica con l'icona matita sul badge (visibile solo finché non c'è ancora una nota) o con un tap sulla nota stessa una volta presente; stringa vuota alla conferma la cancella. I piani salvati con la vecchia rappresentazione (array di soli `ricettaId` stringa, o voci-ricetta senza `nota`) vengono convertiti al volo al caricamento (`migraPiano` in `js/data-model.js`).
- **Trascinamento voci nel Piano**: si può spostare una voce pianificata (ricetta o nota) trascinandola su un'altra cella (stesso giorno o giorno diverso), sia nella vista Piano sia in Oggi. Usa [SortableJS](https://github.com/SortableJS/Sortable) (self-hosted, `js/sortable.min.js`, licenza MIT, nessuna dipendenza), non l'API nativa HTML5 Drag and Drop (che su Chrome/Android non risponde al tocco) né un'implementazione a mano con i soli eventi DOM (provata in un primo momento: inaffidabile su device touch reali, perché il browser può decidere di far scorrere la pagina prima che il JS riesca a intervenire, in base al solo CSS `touch-action`). Un'istanza per cella pasto (`js/components.js`, componente `meal-cell`, hook `mounted`/`beforeDestroy`), tutte nello stesso `group` cosi si trascina anche tra celle/giorni diversi; su touch il trascinamento si attiva solo dopo una breve pressione (`delayOnTouchOnly`), cosi' un semplice scroll non parte per errore. In `onEnd` lo spostamento fatto da Sortable nel DOM viene subito annullato (torna a Vue decidere il DOM finale in base ai dati) e i dati (`piano`) vengono aggiornati tramite `spostaVoceTraCelle` (`js/feature-piano.js`). Attivo se `window.Sortable` è caricato; altrimenti (caricamento fallito, o motore troppo vecchio: l'istanziazione è avvolta in un try/catch) resta disattivato di default, il resto della cella funziona comunque.
- **Duplica voce**: bottone dedicato su ogni voce già pianificata (icona "copia", riusata da quella di copia-giorno) che aggiunge subito nella stessa cella un'altra voce — per le ricette punta alla **stessa** ricetta (nessuna nuova entità nel catalogo) e porta con sé anche l'eventuale `nota` dell'originale; modificando la ricetta, la modifica compare in tutte le voci del piano che la usano; per le note libere copia semplicemente il testo in una voce indipendente. La cella ammette più voci identiche: non c'è nessun controllo di unicità al suo interno.
- **Azioni di una voce: icone o menu "..." a seconda dello schermo**: da "sm" in su (≥576px) le azioni di ogni voce (nota/duplica/rimuovi) restano 3 icone singole nel badge, come sempre. Sotto quella soglia (tutti i telefoni, incluso il Lumia 735 target di questa app) diventano troppo invasive su badge stretti (specie nelle 2 colonne della vista Oggi, sempre affiancate anche su telefono): un solo bottone "menu" (icona `dots-vertical`, 3 puntini verticali) le raggruppa in un menu a comparsa con le stesse voci ma testuali. Nessun plugin Bootstrap JS/Popper (evitato di proposito in tutta l'app): stesso pattern focus/blur di `autocomplete-input` (apre al click/focus del bottone, chiude al blur; le voci di menu usano `@mousedown.prevent` per eseguire l'azione prima che il blur del trigger chiuda il menu) — vedi `.piano-voce-menu` in `css/custom.css` e `openMenuId`/`toggleMenu`/`closeMenu` nel componente `meal-cell`.
- **Copia/incolla a tre livelli**: oltre a giorno intero (`copiaGiorno`/`incollaGiorno`, bottoni nell'header di `day-row`) e settimana intera (`copiaSettimana`/`incollaSettimana`, bottoni sopra l'elenco giorni), si può copiare/incollare anche una singola sezione pasto (`copiaPasto`/`incollaPasto` in `js/feature-piano.js`, bottoni nell'header di ogni `meal-cell`, sia in Piano sia in Oggi). Gli appunti di una sezione non sono vincolati al pasto di origine: si possono incollare in una sezione qualsiasi (es. Colazione copiata e incollata su una Cena), dato che il contenuto (array di voci) ha la stessa forma per tutti e tre i pasti. Tutti e tre i livelli condividono lo stesso `clipboard` (un solo "appunti" alla volta, tipizzato `day`/`week`/`pasto`) e lo stesso pattern: conferma se la destinazione ha già un piano, toast di conferma. Ad ogni copia/incolla le voci vengono clonate in profondità (`clonaVoci`, `JSON.parse(JSON.stringify(...))`) e mai condivise per riferimento: essendo ora oggetti (non più semplici stringhe `ricettaId`), riusare lo stesso riferimento tra più destinazioni farebbe sì che modificare una nota in un punto la cambiasse silenziosamente anche altrove.
...
Con un editor di testo, un "cerca e sostituisci" di `20260913a` su tutto il progetto (compreso `sw.js`) prima di ogni caricamento su Altervista è sufficiente.

## Deploy

Basta caricare l'intera cartella `shopping-kart/` così com'è su un hosting che supporti PHP (es. Altervista — serve solo per `api/sync.php`, la sincronizzazione cloud; senza PHP l'app resta comunque completamente utilizzabile, semplicemente senza login/sync), mantenendo la struttura:
...
│   ├── auth.js             (login/logout via hub di autenticazione configurabile)
│   ├── sync.js             (sincronizzazione cloud dei dati)
│   ├── feature-account.js  (vista account/sync in Impostazioni)
│   └── app.js             (assemblaggio SPA, root Vue, persistenza, backup)
├── api/
│   ├── sync.php            (backend di sincronizzazione, valida il token contro l'hub di autenticazione configurato)
│   ├── .htaccess            (propaga l'header Authorization a PHP)
│   └── data/                (un file JSON per utente, accesso diretto bloccato da .htaccess)
└── icons/
    ├── icon-150.png
    ├── icon-192.png
    ├── icon-512.png
...
- `aggiornatoIl` (epoch ms) è impostato automaticamente da `DataModel.persist()` ad ogni salvataggio: è quello che la sincronizzazione cloud usa per confrontare i dati di questo dispositivo con quelli sul server (vedi sopra, sezione "Account e sincronizzazione cloud"). Il token di accesso, il dominio dell'hub di autenticazione e lo stato di sincronizzazione (`shopping-kart-auth-token-v1`, `shopping-kart-auth-hub-domain-v1`, `shopping-kart-sync-meta-v1`) vivono anch'essi in chiavi dedicate, fuori da questo blob, per lo stesso motivo del tema.
