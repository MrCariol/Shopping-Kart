...
L'app è una Single Page Application (nessun reload di pagina tra una vista e l'altra), con navigazione a icone (senza etichette testuali) su 7 destinazioni: **Lista della spesa**, **Piano alimentare** (settimanale, con ricette e generazione automatica della lista), **Categorie lista/prodotti**, **Categorie ricette**, **Prodotti**, **Ricette** e **Impostazioni** (solo tema scuro + backup).
...
- **Storico acquisti**: in fondo alla vista Lista, un accordion (fatto in casa con `v-if`, non `<details>` nativo — non supportato da EdgeHTML 14) mostra i prodotti già acquistati in passato per riaggiungerli con un tap. Non è un array separato: è `prodotti` filtrato sul campo `ultimoAcquisto` (valorizzato quando un articolo preso viene eliminato dalla lista, singolarmente o in blocco con "Svuota presi").
...
1. Ogni file locale (`css/bootstrap.min.css`, `css/custom.css`, `js/vue.min.js`, tutti i `js/*.js` dell'app, `manifest.json`, le icone) viene richiamato con `?v=20260811a` in fondo all'URL, sia in `index.html` sia dentro `manifest.json`/`browserconfig.xml`.
2. **Prima di ogni pubblicazione**: cerca `20260811a` in tutti i file del progetto e sostituiscilo con un nuovo valore (es. la data del giorno + una lettera, `20260901a`). Cambiando l'URL, il telefono è costretto a scaricare una copia nuova del file anche se il nome resta identico — bypassa la cache indipendentemente da quanto sia aggressiva.
...
Con un editor di testo, un "cerca e sostituisci" di `20260811a` su tutto il progetto prima di ogni caricamento su Altervista è sufficiente.
