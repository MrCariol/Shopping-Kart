...
- **Account e sincronizzazione cloud** (opzionale, in "Impostazioni"): il login avviene tramite l'hub centralizzato [auth.example.invalid](https://auth.example.invalid) (redirect, nessun form di login proprio) — vedi `js/auth.js`. Una volta autenticato, `js/sync.js` confronta i dati locali con quelli salvati sul backend proprio di Shopping Kart (`api/sync.php`, un file JSON per utente in `api/data/`, mai gestito da auth.example.invalid) usando un timestamp `aggiornatoIl`: se solo un lato è cambiato aggiorna l'altro in automatico, se sono cambiati entrambi in modo indipendente mostra un confronto e lascia scegliere all'utente quale versione tenere. La sync è automatica: parte all'avvio, dopo ogni modifica locale (con un breve debounce), quando l'app torna in primo piano (`visibilitychange`/`focus`) e periodicamente mentre resta aperta, così non serve mai aggiornare a mano — l'icona di sync ruota mentre una chiamata è in corso, senza toast, e resta comunque disponibile un "Sincronizza ora" per un tentativo esplicito col risultato mostrato in un toast. Su browser senza `fetch` (Edge 14/Lumia) la sync resta semplicemente disattivata, il resto dell'app funziona come sempre.
...
1. Ogni file locale (`css/bootstrap.min.css`, `css/custom.css`, `js/vue.min.js`, tutti i `js/*.js` dell'app, `manifest.json`, le icone) viene richiamato con `?v=20260828c` in fondo all'URL, sia in `index.html` sia dentro `manifest.json`/`browserconfig.xml`.
2. **Prima di ogni pubblicazione**: cerca `20260828c` in tutti i file del progetto — **incluso `sw.js`** (costante `CACHE_VERSION` in cima al file) — e sostituiscilo ovunque con un nuovo valore (es. la data del giorno + una lettera, `20260901a`). Cambiando l'URL, il telefono è costretto a scaricare una copia nuova del file anche se il nome resta identico; cambiando `CACHE_VERSION`, il Service Worker apre una cache nuova, scarica l'app shell aggiornata e butta via quella vecchia.
...
Con un editor di testo, un "cerca e sostituisci" di `20260828c` su tutto il progetto (compreso `sw.js`) prima di ogni caricamento su Altervista è sufficiente.
...
│   ├── auth.js             (login/logout via auth.example.invalid)
...
├── api/
│   ├── sync.php            (backend di sincronizzazione, valida il token contro auth.example.invalid)
...
- `aggiornatoIl` (epoch ms) è impostato automaticamente da `DataModel.persist()` ad ogni salvataggio: è quello che la sincronizzazione cloud usa per confrontare i dati di questo dispositivo con quelli sul server (vedi sopra, sezione "Account e sincronizzazione cloud"). Il token di accesso e lo stato di sincronizzazione (`shopping-kart-auth-token-v1`, `shopping-kart-sync-meta-v1`) vivono anch'essi in chiavi dedicate, fuori da questo blob, per lo stesso motivo del tema.
