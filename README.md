...
- **Catalogo prodotti nella vista Lista**: un accordion (fatto in casa con `v-if`, non `<details>` nativo — non supportato da EdgeHTML 14) in fondo alla vista Lista mostra l'intero catalogo `prodotti` (nome/unità/categoria editabili, elimina, crea nuovo — le stesse operazioni che prima vivevano in una vista "Prodotti" separata, ora non più in menu) con in più un bottone per aggiungere ciascun prodotto alla lista con un tap. Il campo `prodotti[].ultimoAcquisto` continua a tracciare l'ultimo acquisto (aggiornato quando un articolo preso viene eliminato dalla lista, singolarmente o in blocco con "Svuota presi"), anche se l'accordion ora mostra tutti i prodotti e non solo quelli già comprati.
...
1. Ogni file locale (`css/bootstrap.min.css`, `css/custom.css`, `js/vue.min.js`, tutti i `js/*.js` dell'app, `manifest.json`, le icone) viene richiamato con `?v=20260811d` in fondo all'URL, sia in `index.html` sia dentro `manifest.json`/`browserconfig.xml`.
2. **Prima di ogni pubblicazione**: cerca `20260811d` in tutti i file del progetto e sostituiscilo con un nuovo valore (es. la data del giorno + una lettera, `20260901a`). Cambiando l'URL, il telefono è costretto a scaricare una copia nuova del file anche se il nome resta identico — bypassa la cache indipendentemente da quanto sia aggressiva.
...
Con un editor di testo, un "cerca e sostituisci" di `20260811d` su tutto il progetto prima di ogni caricamento su Altervista è sufficiente.
