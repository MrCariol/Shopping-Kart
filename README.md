...
- **Vista Oggi: due colonne Oggi/Domani**: `oggiData`/`domaniData` (computed in `js/feature-piano.js`, la seconda semplicemente `DataModel.addDays(oggiData, 1)`) alimentano due blocchi affiancati (`col-12 col-sm-6`, classi Bootstrap): da "sm" in su (≥576px) sono due colonne fianco a fianco, sotto quella soglia — quindi su tutti i telefoni, incluso il Lumia 735 target di questa app — tornano impilate a piena larghezza, la stessa larghezza "comoda" che questa vista aveva prima di diventare a due colonne (niente celle strette come nel Piano settimanale). Ogni colonna è indipendente: propria selezione ricetta/nota, proprio drag&drop (stesso `group` Sortable delle altre celle, quindi si trascina anche direttamente da Oggi a Domani).
...
1. Ogni file locale (`css/bootstrap.min.css`, `css/custom.css`, `js/vue.min.js`, tutti i `js/*.js` dell'app, `manifest.json`, le icone) viene richiamato con `?v=20260828b` in fondo all'URL, sia in `index.html` sia dentro `manifest.json`/`browserconfig.xml`.
2. **Prima di ogni pubblicazione**: cerca `20260828b` in tutti i file del progetto — **incluso `sw.js`** (costante `CACHE_VERSION` in cima al file) — e sostituiscilo ovunque con un nuovo valore (es. la data del giorno + una lettera, `20260901a`). Cambiando l'URL, il telefono è costretto a scaricare una copia nuova del file anche se il nome resta identico; cambiando `CACHE_VERSION`, il Service Worker apre una cache nuova, scarica l'app shell aggiornata e butta via quella vecchia.
...
Con un editor di testo, un "cerca e sostituisci" di `20260828b` su tutto il progetto (compreso `sw.js`) prima di ogni caricamento su Altervista è sufficiente.
