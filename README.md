...
- **Duplica ricetta**: bottone dedicato su ogni ricetta già pianificata (icona "copia", riusata da quella di copia-giorno) che aggiunge subito nella stessa cella un'altra voce che punta alla **stessa** ricetta (nessuna nuova entità nel catalogo), utile ad es. per una doppia porzione — modificando la ricetta, la modifica compare in tutte le voci del piano che la usano. La cella ammette più voci identiche: non c'è più un controllo di unicità sulla ricetta all'interno della cella.
...
1. Ogni file locale (`css/bootstrap.min.css`, `css/custom.css`, `js/vue.min.js`, tutti i `js/*.js` dell'app, `manifest.json`, le icone) viene richiamato con `?v=20260816d` in fondo all'URL, sia in `index.html` sia dentro `manifest.json`/`browserconfig.xml`.
2. **Prima di ogni pubblicazione**: cerca `20260816d` in tutti i file del progetto — **incluso `sw.js`** (costante `CACHE_VERSION` in cima al file) — e sostituiscilo ovunque con un nuovo valore (es. la data del giorno + una lettera, `20260901a`). Cambiando l'URL, il telefono è costretto a scaricare una copia nuova del file anche se il nome resta identico; cambiando `CACHE_VERSION`, il Service Worker apre una cache nuova, scarica l'app shell aggiornata e butta via quella vecchia.
...
Con un editor di testo, un "cerca e sostituisci" di `20260816d` su tutto il progetto (compreso `sw.js`) prima di ogni caricamento su Altervista è sufficiente.
