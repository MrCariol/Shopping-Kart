...
- **Voci del piano: ricette o note libere**: ogni cella pasto è un array di "voci" (`{id, tipo:'ricetta', ricettaId}` oppure `{id, tipo:'nota', testo}`, vedi commento in testa a `js/feature-piano.js`), non più solo ricette. Una nota è testo libero (es. "Pizza con amici fuori") per i pasti che non vale la pena modellare come ricetta/ingredienti veri: si aggiunge dallo stesso modale "Aggiungi al piano" (campo separato sotto la ricerca ricetta), si modifica con un tap sul testo (editor inline nella cella), ed è fine a se stessa — non genera ingredienti in "Genera lista" e se rimossa dal piano è persa per sempre (nessun salvataggio in un catalogo, a differenza delle ricette). I piani salvati con la vecchia rappresentazione (array di soli `ricettaId` stringa) vengono convertiti al volo al caricamento (`migraPiano` in `js/data-model.js`).
...
- **Duplica voce**: bottone dedicato su ogni voce già pianificata (icona "copia", riusata da quella di copia-giorno) che aggiunge subito nella stessa cella un'altra voce — per le ricette punta alla **stessa** ricetta (nessuna nuova entità nel catalogo), utile ad es. per una doppia porzione: modificando la ricetta, la modifica compare in tutte le voci del piano che la usano; per le note copia semplicemente il testo in una voce indipendente. La cella ammette più voci identiche: non c'è nessun controllo di unicità al suo interno.
...
1. Ogni file locale (`css/bootstrap.min.css`, `css/custom.css`, `js/vue.min.js`, tutti i `js/*.js` dell'app, `manifest.json`, le icone) viene richiamato con `?v=20260828a` in fondo all'URL, sia in `index.html` sia dentro `manifest.json`/`browserconfig.xml`.
2. **Prima di ogni pubblicazione**: cerca `20260828a` in tutti i file del progetto — **incluso `sw.js`** (costante `CACHE_VERSION` in cima al file) — e sostituiscilo ovunque con un nuovo valore (es. la data del giorno + una lettera, `20260901a`). Cambiando l'URL, il telefono è costretto a scaricare una copia nuova del file anche se il nome resta identico; cambiando `CACHE_VERSION`, il Service Worker apre una cache nuova, scarica l'app shell aggiornata e butta via quella vecchia.
...
Con un editor di testo, un "cerca e sostituisci" di `20260828a` su tutto il progetto (compreso `sw.js`) prima di ogni caricamento su Altervista è sufficiente.
